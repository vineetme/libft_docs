...
the hidden "stop sign" at the end of the string.
...
Then why don't we add if (!s) return (0); to protect against NULL?

Because the standard C library strlen does not protect against NULL. If you 
pass NULL to the real strlen, it segfaults. The goal of Libft is to mimic the 
original functions exactly, unless subject files states otherwise explicitly.
...
Sets starting position to the very first character of the string (index 0).
...
This translates to: "As long as the current character is NOT the 
null-terminator, keep doing the code inside the block."
...
In languages like Python or Java, a string is an object that knows its 
own length. In C, a string is literally just a line of consecutive bytes 
in memory. The computer has absolutely no idea how long the string is.

The Null Terminator ('\0')
Because the computer doesn't know the length, the creators of C decided 
on a universal rule: Every string must end with a byte composed entirely 
of zeros. In the ASCII table, the character value of 0 is represented as 
'\0' (the Null character).

Why size_t instead of int?
An int can be negative. Can a string have a length of -5? No.
Also, a standard int usually maxes out at around 2.1 billion. On a modern 
64-bit machine, size_t can hold a number up to 18 quintillion. If you have 
a massive text file loaded into memory, int might overflow, but size_t is 
guaranteed by the compiler to be large enough to measure any object your 
